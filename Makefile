...
test-functional: docker-build
	...
	docker run --rm --name test-presidio-recognizers-store --network testnetwork -d -p 3004:3004 -e GRPC_PORT=3004 $(DOCKER_REGISTRY)/presidio-recognizers-store:$(PRESIDIO_LABEL)
	sleep 30
